#!/usr/bin/env python3
"""Insert (or refresh) reference rows for testing tiered healthier substitutes.

Deletes any existing rows previously inserted by this script (``classification_method = seed_substitute_tests``), then loads
``data/substitute_recommender_test_products.csv`` into
``catalog.reference_products`` (see ``REFERENCE_CATALOG_*`` env).

Does **not** truncate the full table — other reference products are left intact.

After running, restart the API process (or call code that clears in-memory substitute
catalog caches) so ``build_healthier_substitutes`` reloads from PostgreSQL.

Usage (from repo root)::

    python scripts/seed_substitute_recommender_tests.py
"""
...
_SEED_MARKER = "seed_substitute_tests"


def main() -> None:
    csv_path = _ROOT / "data" / "substitute_recommender_test_products.csv"
            ...
            rows.append(
                {
                    ...
                    "classification_method": _SEED_MARKER,
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text(
                f"DELETE FROM {dest} "
                "WHERE classification_method = :marker"
            )
            .bindparams(marker=_SEED_MARKER)
        )
        for row in rows:
    ...
    print(f"Seeded substitute-test reference rows: {n} (restart app to refresh substitute cache).")
