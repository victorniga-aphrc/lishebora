"""
Normalize catalog / label product lines for matching.

Strips pack sizes and common packaging tokens so the same logical product (per 100g/ml)
maps to one key; used by lookup generation and runtime name matching against the reference table.
"""
...
def _apply_lexical_normalization(s: str) -> str:
    """Remove export/shelf noise tokens; expand CHOC/BISC for matching."""
    s = _REMOVE_EOT.sub(" ", s)
